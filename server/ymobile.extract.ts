...
const ymobile = async (req: any, res) => {
    ...
    // basic usage
    const billingContent = findString(html, '<div id="bills">', '<div id="information">');

    const buContent = findString(billingContent, '<h5 class="clearfix"><span class="name">基本料', '<h5 class="clearfix"');
    let items: any = [];
    let item: any = {};
    buContent.split(/\n/).forEach(line => {
        ...
        if (key)
            ...
            if (value) {
                ...
                value = /<td(.*?)>([+-]?\$?[0-9]\d?(?:,*\d{3})*(?:\.\d{2})?)/.exec(line);
                if (value) {
                    item.value = value[2].trim();
                    items.push(item);
                    item = {};
    ...
    const dataContent = findString(billingContent, '<h5 class="clearfix"><span class="name">データ定額料', '<h5 class="clearfix"');
    items = [];
    item = {};
    dataContent.split(/\n/).forEach(line => {
        const key = /<th>(.*?)<\/th>/.exec(line);
    ...
    const commContent = findString(billingContent, '<h5 class="clearfix"><span class="name">通話料', '<h5 class="clearfix"');
    items = [];
    item = {};
    commContent.split(/\n/).forEach(line => {
        const key = /<th>(.*?)<\/th>/.exec(line);
