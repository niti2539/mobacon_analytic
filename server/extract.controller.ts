...
import { docomo } from "./docomo.extract";
...
router.post("/softbank/:correlationid", async (req: any, res) => {
  ...
  // communication fee
  const optionContent = findString(
    html,
    '<em id="option-services">オプションサービス料',
    "</table"
  );
  items = [];
  item = {};
  ...
  // communication fee
  const universalContent = findString(
    html,
    "<em>ユニバーサルサービス料</em>",
    "</table"
  );
...
router.post("/docomo/:correlationid", docomo);
